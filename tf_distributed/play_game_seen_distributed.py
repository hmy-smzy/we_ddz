...
from training.cnn_structure import conv_net_seen
from utils.input_trans_seen import play_game_input_seen, complement
...
class PlayGameSeen(object):
    ...
    def _init_graph(self):
        ...
        with self._tf_graph.as_default(), tf.device("/job:worker%s" % self.device):
            with tf.name_scope("play_seen"):
                x = tf.placeholder(tf.float32, [22, 43, 15], name='x_input')
    ...
    def get_game_result(self, game_ary, pot, turn=None, process=None, random_play=False):
        ...
        for p in out_hands:
            cur_cards[p[0]] -= p[2]
        while True:
            x_input, score = play_game_input_seen(game_ary, out_hands, role)
            if isinstance(x_input, int):
                ...
                top_n_value = self._tf_session.run(self._tf_graph.get_collection('pred_top'),
                                                   feed_dict={self._tf_graph.get_tensor_by_name("play_seen/x_input:0"): x_input,
                                                              self._tf_graph.get_tensor_by_name(
                                                                  'play_seen/legal_label:0'): score})[0]
    ...
    def get_one_hand(self, cards, process, role, pot, random_play=False):
        x_input, legal_label = play_game_input_seen(cards, process, role)
        if isinstance(x_input, int):
            return HAND_LABEL2CHAR[x_input], legal_label, 0
        else:
            pt, score_rate = self._tf_session.run([self._tf_graph.get_collection('pred_top'),
                                                   self._tf_graph.get_collection('score')],
                                                  feed_dict={self._tf_graph.get_tensor_by_name("play_seen/x_input:0"): x_input,
                                                             self._tf_graph.get_tensor_by_name(
                                                                 'play_seen/legal_label:0'): legal_label})
            out_hand_type, prob = self._get_hand(pt[0], legal_label, random_play)
            if 130 <= out_hand_type <= 223 or 269 <= out_hand_type <= 294:
                hand = HAND_LABEL2CHAR[out_hand_type]
                ...
                out_hand = HAND_LABEL2CHAR[out_hand_type]
            return out_hand, prob, score_rate[0][0][0]
    ...
    def get_top_n_hand(self, cards, process, role, pot):
        ...
        if isinstance(x_input, int):
            ...
            for i in range(self.top_n):
                if top_n_value[0][0][i] > 0:
                    if 130 <= top_n_value[1][0][i] <= 223 or 269 <= top_n_value[1][0][i] <= 294:
                        ...
                        out_kicker, kicker_str = self.get_kicker(cards[role], pot, process, role, hand)
                        check_legal = np.copy(cards[role])
                        for p in process:
                            if p[0] == role:
                                check_legal -= p[2]
                        check_legal -= str2ary(hand[:-1])
            ...
            return out_hands, probs, score_rate[0][0][0]
...
if __name__ == '__main__':
    ...
    deck = PlayGameSeen('../play_model_seen', '../kicker_model')
    ...
    oo = deck.get_top_n_hand(game_ary, process, role, game_ary[3])
    print(oo)
"""
    # game_ary[0] += game_ary[3]
    # t1 = time.time()
