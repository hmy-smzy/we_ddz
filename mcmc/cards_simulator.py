...
from mcmc.hand_prob import CalHandProb
...
def __simulate_initial_cards_task(cards,
    ...
    down_ai_engine = CalHandProb(down_ai_model_path)
    up_ai_engine = down_ai_engine if down_ai_model_path == up_ai_model_path else CalHandProb(up_ai_model_path)
    ...
    for i in range(num_processes):
        p = Process(target=__simulate_initial_cards_task,
                    ...
                    kwargs={'num_simulation_step': num_simulation_step, 'num_loop': int(num_loop / num_processes), 'p0': p0,
                            'lord_seat': lord_seat, 'style': style}
                    )
        processes.append(p)
