...
class Deck(object):
    ...
    def print_info(self):
        ...
        prt = '%s lord=%d; point=%d; first_call=%d; %s; %r' % (card_str, self.lord, self.point, self.first_call, process_str, self.score)
        print(prt)
        return prt
    ...
    def run_an_endgame(self, sorted_cards, process, point, turn, random_play=False, pot=None):
        ...
        if is_game_over > -1:
            ...
            self.win, self.game_process = self.PlayGame.get_game_result(sorted_cards, pot, turn=turn, process=process, random_play=random_play)
        self._reward()
        # self.print_info()
        return self.score
...
if __name__ == '__main__':
    ...
    first_call = 0
    env = Deck('../play_model', '../kicker_model')
    env.random_a_game()
